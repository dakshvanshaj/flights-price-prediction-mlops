...
# --- NEW: Import our robust data loader ---
...
def split_data():
    ...
    # --- REFACTORED: Use load_data instead of pd.read_csv ---
    df = load_data(str(INPUT_CSV_PATH))

    if df is None:
        logger.error(f"Failed to load data from '{INPUT_CSV_PATH}'. Aborting process.")
        return
