...
from gold_data_preprocessing.feature_engineering import create_cyclical_features
...
def gold_engineering_pipeline(
    ...
    # === STAGE 4: CYCLIC FEATURE ENCODING ===
    logger.info("=" * 25 + " STAGE 4/5: CYCLIC FEATURE ENCODING " + "=" * 25)
    df = create_cyclical_features(df, cyclical_map=config_gold.CYCLICAL_MAP)
...
def main():
    ...
    train_success, fitted_imputer = gold_engineering_pipeline(
        input_filepath=train_path
    )  # re add fitted_encoder once feature engineering is done
